...
def run_inference_stream():
    print(f"[Client] Connecting to Magna Inference Server at {SERVER_ADDRESS}...")
    ...
    # Health check
    try:
        health_resp = stub.HealthCheck(magna_pb2.HealthRequest(), timeout=2.0)
        print(f"[Client] Server is ready! (Backend: {health_resp.backend}, Model: {health_resp.model_name})")
    except grpc.RpcError as e:
        print(f"[Client] Failed to connect to server: {e}")
        return

    # Open the camera stream
    print(f"[Client] Opening camera stream on {CAMERA_DEVICE}...")
    cap = cv2.VideoCapture(CAMERA_DEVICE, cv2.CAP_V4L2)
    
    # Try setting MJPG format for better performance if supported
    ...
    print("[Client] Starting live streaming. Capturing 100 frames to 'inference_output.mp4'...")

    # Set up VideoWriter
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out_video = cv2.VideoWriter('inference_output.mp4', fourcc, 15.0, (1280, 720))
    ...
    try:
        while frame_count < 100:
            ...
            # 2. Compress the frame to JPEG to save network bandwidth over gRPC
            ...
            request = magna_pb2.InferenceRequest(
                ...
                format="jpeg"  # Tell the server it's receiving a compressed JPEG
            )

            # 4. Call the Magna Inference Server
            try:
                response = stub.Infer(request)
                
                # Calculate True FPS (including capture, network, and inference)
                total_time_ms = (time.time() - start_time) * 1000
                fps = 1000.0 / total_time_ms

                # 5. Format and Output Correctly onto the Video Frame!
                # Draw the Top-1 Prediction in large text
                if response.top1:
                    class_idx_str = str(response.top1.class_index)
                    if class_idx_str in IMAGENET_LABELS:
                        label_name = IMAGENET_LABELS[class_idx_str].replace('_', ' ')
                    else:
                        label_name = response.top1.label if response.top1.label else "Unknown"
                        
                    label_text = f"Class {response.top1.class_index}: {label_name.title()} ({response.top1.score * 100:.1f}%)"
                    
                    # Draw a nice semi-transparent black background box for the text
                    overlay = frame.copy()
                    ...
                    # Sub-text for remaining Top-5 (if available)
                    y_offset = 95
                    for i, pred in enumerate(response.top5[1:3]): # Just showing next 2 for space
                        pred_idx_str = str(pred.class_index)
                        if pred_idx_str in IMAGENET_LABELS:
                            sub_label = IMAGENET_LABELS[pred_idx_str].replace('_', ' ')
                        else:
                            sub_label = pred.label if pred.label else "Unknown"
                            
                        sub_text = f"#{i+2}: class {pred.class_index}: {sub_label.title()} ({pred.score * 100:.1f}%)"
                        cv2.putText(frame, sub_text, (20, y_offset), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                        y_offset += 25
                
                # Add FPS counter to top right
                fps_text = f"True FPS: {fps:.1f} | Server Time: {response.inference_time_ms:.1f}ms"
                cv2.putText(frame, fps_text, (frame.shape[1] - 500, 40), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

            except grpc.RpcError as e:
                print(f"Inference error: {e.details()}")

            # 6. Write the frame to the video file
            out_video.write(frame)
            frame_count += 1
            if frame_count % 10 == 0:
                print(f"[Client] Captured {frame_count}/100 frames...")

    except KeyboardInterrupt:
        print("\n[Client] Stopping camera stream...")
    finally:
        cap.release()
        out_video.release()
        print("[Client] Wrote 100 inference frames to 'inference_output.mp4'")
